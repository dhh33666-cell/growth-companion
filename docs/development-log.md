...
## 2026-08-19 - 云端同步数据保护修正

- Status: Completed
- Goal: 修正上一版重复同步修复中仍保留的“先删除后写入”风险，避免读取异常时将每日收获误同步为空，并清理重复奖励。
- Completed: 移除任务、生活记录、奖励和记忆的全表先删后写；改为先 upsert 当前数据，再只删除云端中不再存在的指定行；保留云端 UUID 作为稳定 ID；奖励读取时按内容去重；每日收获查询发生错误时改为抛出错误并保留本地数据；浏览器新增最近 10 份本地数据快照，并在设置页提供“尝试恢复本机备份”。
- Validation: TypeScript 检查通过；Vitest 6/6 通过；Next.js production build 通过。
- Problems: 在此修正发布前，上一版线上代码已可能将部分用户的每日收获写为空；若同一浏览器没有旧本地快照、其他设备缓存或自行导出的 JSON，无法从应用代码直接还原已经被删除的云端内容。
- Decision: 后续涉及云端同步的修改必须先避免破坏性全量删除，并优先增加恢复路径与跨设备验证。
- Next: 立即发布此修正；部署后检查奖励自动去重。对于已丢失的每日收获，优先检查其他设备/本地地址是否仍保留旧缓存并手动恢复。

## 2026-08-26 - 逾期任务延期回到今日
